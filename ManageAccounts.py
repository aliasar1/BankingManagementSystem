...
class ManageAccounts(ManageBankFiles):
    ...
    def openAccount(self, file):
        ...
        checker = "^(?:(?:31(\\/|-|\\.)(?:0?[13578]|1[02]))\\1|(?:(?:29|30)(\\/|-|\\.)(?:0?[13-9]|1[0-2])\\2))(?:(?:1[6-9]|[2-9]\\d)?\\d{2})$|^(?:29(\\/|-|\\.)0?2\\3(?:(?:(?:1[6-9]|[2-9]\\d)?(?:0[48]|[2468][048]|[13579][26])|(?:(?:16|[2468][048]|[3579][26])00))))$|^(?:0?[1-9]|1\\d|2[0-8])(\\/|-|\\.)(?:(?:0?[1-9])|(?:1[0-2]))\\4(?:(?:1[6-9]|[2-9]\\d)?\\d{2})$"
        ...
        while True:
            self.accType = input(
                "Enter your account type(S = Saving Account/ C = Current Account): ").upper()
            if self.accType == "S":
                self.accType = "Saving"
                break
            elif self.accType == "C":
        ...
        while True:
            ...
            if not re.match(checker, self.dob):
                print(
                    "Error! Make sure you only use / and numbers in your Date of Birth.")
            else:
                break
        while True:
            self.gender = input(
            ...
            try:
                while True:
                    self.ammount = int(input(
                        "Please enter your initial ammount to depsoit for opening account(Atlest Rs 500 for saving account and Atleat 1000 for current account): "))
    ...
    def deleteAccount(self, file, file2):
        if super().extractAccount(file, file2) == False:
            return
        self.accNum = input(
            "Please enter your account number to delete account: ")
        self.flag = False
        with open(file, 'r') as f:
            accounts = f.readlines()
            for account in accounts:
    ...
    def modifyDetails(self, file, file2):
        ...
        if super().extractAccount(file, file2) == False:
            return
        self.accNum = input(
            "Please enter your account number to modify the account data: ")
        self.flag = False
        with open(file, 'r') as f:
            accounts = f.readlines()
            for account in accounts:
                ...
                if account[0] == self.accNum:
                    ...
                    while True:
                        try:
                            self.md = int(input(
                                "What is the data you want to modify: \n1. Name\n2. Date Of Birth\n3. Gender\n4. Address\n5. Contact Number\n6. Account Type\n"))
                            if self.md == 1:
                                while True:
                                    self.newName = input(
                                        "Enter your new name: ")
                                ...
                                while True:
                                    ...
                                    if not re.match(checker, self.newDOB):
                                        print(
                                            "Error! Make sure you only use / and numbers in your Date of Birth.")
                                    else:
                                ...
                                while True:
                                    self.accType = input(
                                        "Enter your new account type(S = Saving Account/ C = Current Account): ").upper()
                                    if self.accType == "S":
                                        account[7] = "Saving\n"
                                        break
                                    elif self.accType == "C":
