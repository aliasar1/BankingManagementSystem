...
class Transaction(ManageBankFiles):
    ...
    def checkBalance(self, file):
        if os.path.exists(file) == False:
            ...
            return
        self.accNum = input(
            "Please enter your account number to check its balance: ")
            ...
            for account in accounts:
                ...
                if account[0] == self.accNum:
                    print(
                        "-----------------------------------------------------------------------")
                    print(
                        f"Account balance of {self.accNum} is Rs {account[6]}")
                    print(
                        "-----------------------------------------------------------------------")
                    return
    ...
    def depositAccount(self, file, file2):
        if super().extractAccount(file, file2) == False:
            return
        self.accNum = input(
            "Please enter your account number to deposit money in your account: ")
        self.flag = False
        with open(file, 'r') as f:
            accounts = f.readlines()
            for account in accounts:
                ...
                if account[0] == self.accNum:
                    while True:
                        while True:
                            try:
                                self.money = int(
                                    input("Enter how much money you want to depsoit in your account: "))
                                break
                            except Exception:
    ...
    def withdrawAccount(self, file, file2):
        if super().extractAccount(file, file2) == False:
            return
        self.accNum = input(
            "Please enter your account number to withdraw money from your account: ")
        self.flag = False
        with open(file, 'r') as f:
            accounts = f.readlines()
            for account in accounts:
                ...
                if account[0] == self.accNum:
                    if (account[7] == "Saving\n" and account[6] == "500") or (account[7] == "Current\n" and account[6] == "1000"):
                        print(
                            "You cannot withdraw money as you have insufficient balance. Please depsoit some money first.")
                        os.remove(file2)
                        return
                    while True:
                        while True:
                            try:
                                self.money = int(
                                    input("Enter how much money you want to withdraw in your account: "))
                                self.bal = int(account[6]) - self.money
                                if (account[7] == "Saving\n" and self.bal >= 500) or (account[7] == "Current\n" and self.bal >= 1000):
                                    break
                                else:
                                    if (account[7] == "Saving\n"):
                                        self.validAmmount = int(
                                            account[6]) - 500
                                    ...
                                    print(
                                        "-----------------------------------------------------------------------")
                                    print(
                                        "Insufficient balance in your account. Please enter lesser money to withdraw.")
                                    print(
                                        "Atlest Rs 500 for saving account and atleat 1000 for current account must remain as current balance.")
                                    print(
                                        f"Your current balance now is only Rs {account[6]} which means you can only withdraw Rs {self.validAmmount} or less.")
                                    print(
                                        "-----------------------------------------------------------------------")
                                    print()
                            except Exception:
